...
namespace AdsWrapper
{
    EtcDeviceWrapper::EtcDeviceWrapper(String^ remoteIp, String^ remoteNetId)
    {
        ...
        try
        {
            NativeLogger::Instance().Log(NativeLogger::LogLevel::Debug,
                "EtcDeviceWrapper: Setting up EtherCAT master access");
            ...
            NativeLogger::Instance().Log(NativeLogger::LogLevel::Debug,
                "EtcDeviceWrapper: SetupEtcMaster completed successfully");
        }
        catch (const std::exception& ex)
        {
            NativeLogger::Instance().Log(NativeLogger::LogLevel::Error,
                "EtcDeviceWrapper: SetupEtcMaster failed - " + std::string(ex.what()));
    ...
    EtcDeviceWrapper::!EtcDeviceWrapper()
    {
        ...
        {
            NativeLogger::Instance().Log(NativeLogger::LogLevel::Debug,
                "EtcDeviceWrapper: Releasing native resources");
            delete _native;
            _native = nullptr;
        }
    }
    ...
    {
        if (_disposed)
            throw gcnew ObjectDisposedException("EtcDeviceWrapper");
    }
    ...
    String^ EtcDeviceWrapper::ListAllEtcMasters()
    {
        ...
        {
            NativeLogger::Instance().Log(NativeLogger::LogLevel::Debug,
                "EtcDeviceWrapper: Listing all EtherCAT masters");
            std::ostringstream oss;
            long status = _native->ListAllEtcMasters(oss);
            if (status)
            {
                NativeLogger::Instance().Log(NativeLogger::LogLevel::Error,
                    "EtcDeviceWrapper: ListAllEtcMasters failed with status " + std::to_string(status));
                throw gcnew Exception("ListAllEtcMasters failed with status " + status.ToString());
            }

            std::string result = oss.str();
            NativeLogger::Instance().Log(NativeLogger::LogLevel::Debug,
                "EtcDeviceWrapper: ListAllEtcMasters completed successfully");
            return gcnew String(result.c_str());
        }
        catch (const std::exception& ex)
        {
            NativeLogger::Instance().Log(NativeLogger::LogLevel::Error,
                "EtcDeviceWrapper: ListAllEtcMasters failed - " + std::string(ex.what()));
            throw gcnew Exception(gcnew String(ex.what()));
        }
    }
    ...
    array<ECatMasterSlaveStatus>^ EtcDeviceWrapper::GetECatSlaveAlStatus()
    {
        ...
        {
            NativeLogger::Instance().Log(NativeLogger::LogLevel::Debug,
                "EtcDeviceWrapper: Getting EtherCAT slave AL status");
            auto nativeResult = _native->GetECatSlaveAlStatus();
            if (nativeResult.empty())
            {
                NativeLogger::Instance().Log(NativeLogger::LogLevel::Debug,
                    "EtcDeviceWrapper: No EtherCAT slave status found");
            ...
            for (const auto& pair : nativeResult)
            {
                ...
                if (statuses.empty())
                {
                    NativeLogger::Instance().Log(NativeLogger::LogLevel::Warning,
                        "EtcDeviceWrapper: No slave AL status found for master " +
            ...
            NativeLogger::Instance().Log(NativeLogger::LogLevel::Debug,
                "EtcDeviceWrapper: GetECatSlaveAlStatus completed successfully, found " +
        ...
        {
            NativeLogger::Instance().Log(NativeLogger::LogLevel::Error,
                "EtcDeviceWrapper: GetECatSlaveAlStatus failed - " + std::string(ex.what()));
            throw gcnew Exception(gcnew String(ex.what()));
        }
    }
}